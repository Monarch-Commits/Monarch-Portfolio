...
export default function DashboardImage() {
  return (
    <div className="relative flex w-full max-w-md items-center justify-center p-6 md:max-w-md lg:max-w-lg">
      {/* Background blobs */}
      <div className="absolute top-10 -left-10 z-0 h-40 w-40 rotate-12 rounded-[45%_55%_50%_50%] bg-green-400/50 blur-3xl" />
      <div className="absolute top-4 right-0 z-0 h-16 w-16 rounded-full bg-green-400/40 blur-2xl" />
      <div className="absolute bottom-0 left-1/4 z-0 h-20 w-20 rotate-6 rounded-lg bg-green-800/40 blur-2xl" />

      {/* Main image container */}
      <div className="relative z-10 flex aspect-square w-3/4 max-w-sm flex-col items-center justify-end overflow-hidden rounded-[69%_31%_24%_76%/79%_66%_34%_21%] bg-linear-to-tr from-green-950 sm:h-72 sm:w-72 sm:max-w-md md:h-70 md:w-70 md:max-w-lg">
        <Image
      ...
      {/* Decorative circles */}
      <div className="absolute top-1/2 right-4 z-10 h-12 w-12 rounded-full border-2 border-green-600 blur-sm" />
      <div className="absolute bottom-2 left-10 z-10 h-8 w-8 rounded-full bg-green-300/50 blur-sm" />
    </div>
  );
}
